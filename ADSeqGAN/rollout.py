...
class Rollout(object):
    ...
    def __init__(self, lstm, update_rate, pad_num):
        ...
        self.temperature = self.lstm.temperature
        ...
        # # When current index i < 2, add noise to the input
        # def _g_recurrence_0(i, x_t, h_tm1, gen_x):
        #     h_t = self.g_recurrent_unit(x_t, h_tm1)  # hidden_memory_tuple
        #     x_tp1 = ta_emb_x.read(i)
        #     noise = tf.random_normal(tf.shape(x_tp1), mean=0.0, stddev=0.04)
        #     x_tp1 = x_tp1 + noise
        #     gen_x = gen_x.write(i, ta_x.read(i))
        #     return i + 1, x_tp1, h_t, gen_x

        # When 2 <= current index i < given_num, use the provided tokens as the input at each time step
        ...
        # # First loop: handle first 2 characters with noise
        # i, x_t, h_tm1, self.gen_x = control_flow_ops.while_loop(
        #     cond=lambda i, _1, _2, _3: i < 2,
        #     body=_g_recurrence_0,
        #     loop_vars=(tf.constant(0, dtype=tf.int32),
        #               tf.nn.embedding_lookup(self.g_embeddings, self.start_token), 
        #               self.h0, 
        #               gen_x))

        # Second loop: handle characters from index 2 to given_num without noise
        ...
        # Third loop: handle remaining characters
    ...
    def get_reward(self, sess, input_x, rollout_num, dis, reward_fn=None, D_weight=1):
        ...
        for i in range(rollout_num):
            ...
            for given_num in range(1, self.sequence_length):
                ...
                if reward_fn:
                    ...
                    #     # MinMax标准化当前batch的奖励到[0,1]区间
                    #     if len(rew) > 1:
                    #         rew_min = np.min(rew)
                    #         rew_max = np.max(rew)
                    #         if rew_max > rew_min:
                    #             rew = (rew - rew_min) / (rew_max - rew_min)
                    ...
                    # already = sorted(already, key=lambda el: el[0])
                        rew = reward_fn(generated_seqs)
                        all_rewards.extend(rew)  # 收集原始奖励
                        
                        # 使用zip来同时遍历gind和rew
                        for idx, (k, r) in enumerate(zip(gind, rew)):
                            ypred[k] += reward_weight * r  # 使用原始奖励
                ...
                if i == 0:
                    # rewards.append(ypred)
                    raw_rewards.append(ypred)
                else:
                    # rewards[given_num - 1] += ypred
                    raw_rewards[given_num - 1] += ypred
            ...
            if reward_fn:
                ...
                final_rew = reward_fn(input_x)
                all_rewards.extend(final_rew)
            #     # MinMax标准化最终奖励到[0,1]区间
            #     if len(final_rew) > 1:
            #         rew_min = np.min(final_rew)
            #         rew_max = np.max(final_rew)
            #         if rew_max > rew_min:
            #             final_rew = (final_rew - rew_min) / (rew_max - rew_min)
            #     ypred += reward_weight * final_rew
            # else:
            #     ypred = np.array([item[1] for item in ypred_for_auc])             
                # Choose the seqs finished in the last iteration
                # for j, k in enumerate(gind):
                #     if input_x[k][given_num] == self.pad_num and input_x[k][given_num-1] == self.pad_num:
                #         already.append((k, final_rew[j]))
                # already = sorted(already, key=lambda el: el[0])
                ypred += reward_weight * final_rew
            else:
                ypred = np.array([item[1] for item in ypred_for_auc])
        ...
        if len(all_rewards) > 0:
            ...
            rewards_min = np.min(raw_rewards)
            rewards_max = np.max(raw_rewards)
            if rewards_max > rewards_min:
        #         rewards = (rewards - rewards_min) / (rewards_max - rewards_min)

        # rewards = np.transpose(rewards) / (1.0 * rollout_num)  # batch_size x seq_length
                rewards = (raw_rewards - rewards_min) / (rewards_max - rewards_min)
            else:
                rewards = raw_rewards
        else:
            rewards = raw_rewards
        
        rewards = np.transpose(rewards) / (1.0 * rollout_num)
        return rewards
